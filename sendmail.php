 ...
 */
function getTopText () {
    $message = "Du modtog en ny besked sendt via kontaktformularen på the-international-artworkshop.dk.";
    $message = strlen($message) > 0 ? $message : "You received a new message sent via the contact form on the-international-artworkshop.dk.";
...
function getMailHeaders ($from, $replyTo, $boundary) {
    $headers = array(
        ...
        "Message-ID: <" . implode("@", [md5(uniqid(time())), "the-international-artworkshop.dk"]) . ">",
        "Date: ".date("r (T)"),
        "X_Mailer: PHP/" . phpversion(),
        "X-G1i-WSB-Sendmail: 113.0.29",
    );
...
function getDebugInfo() {
    ...
    if (isValidRequester() || $debugHeader === "the-international-artworkshop.dk") {
